...
const baseDir = 'C:\ProgramData\01Agent\wsl';
const imagePath = path.join(baseDir, 'neuralos.tar.gz');
const imageUrl = 'https://01agent-website-s3.s3.us-east-1.amazonaws.com/uploads/neuralos.tar.gz';
...
function installWSLWithElevation() {
  return new Promise((resolve) => {
    const options = { name: '01Agent Setup' };
...
export async function setupBackgroundMode({ onStatus, onProgress }) {
  try {
    if (!checkWindowsVersion()) {
      throw new Error('❌ 01Agent background mode requires Windows 10 or later.');
    }
    ...
    if (!wsl || !vm) {
      throw new Error(reboot
        ? '✅ Features enabled. Please reboot your PC and relaunch 01Agent.'
    ...
    if (!isDistroRegistered()) {
      log('Downloading 01Agent Runtime Environment...');
      await downloadImage(onProgress);
      log('Importing 01Agent Runtime Environment...');
      importDistro();
    }

    // --- Verify ---
    log('Verifying 01Agent Runtime Environment setup...');
    verifyDistro();
    log('✅ 01Agent Runtime Environment is ready.');

import logging, time, os, subprocess, pyautogui, pyperclip, platform, webbrowser, ctypes, pyttsx3, threading, json, psutil, mss
try: import pywinctl as pwc
except ImportError: pwc = None
logger = logging.getLogger(__name__)
pyautogui.FAILSAFE = True

class Executor:
    def __init__(self):
        self.system = platform.system().lower()
        self._scale_factor = self._get_dpi_scale()
        self._monitors = mss.mss().monitors
        try:
            self._tts_engine = pyttsx3.init()
            self._tts_engine.setProperty('rate', 150)
        except Exception:
            self._tts_engine = None

    def _get_dpi_scale(self):
        if self.system == "windows":
            try:
                hdc = ctypes.windll.user32.GetDC(0)
                dpi = ctypes.windll.gdi32.GetDeviceCaps(hdc, 88)
                ctypes.windll.user32.ReleaseDC(0, hdc)
                return dpi / 96.0
            except Exception: return 1.0
        elif self.system == "darwin":
            # macOS typically handles scaling internally, but for automation coordinates
            # we often need to check the backing scale factor.
            # PyAutoGUI handles this mostly on modern versions.
            return 1.0
        return 1.0

    def _map_coords(self, x, y):
        if self.system == "windows":
            return int(x / self._scale_factor), int(y / self._scale_factor)
        return x, y

    def execute_actions(self, actions):
        results = []
        for action_obj in actions:
            action = action_obj.get("action")
            params = action_obj.get("params", {})
            res = {"action": action, "status": "success"}
            try:
                if action == "mouse_move":
                    x, y = self._map_coords(params.get("x"), params.get("y"))
                    pyautogui.moveTo(x, y, duration=0.1)
                elif action == "left_click":
                    x, y = self._map_coords(params.get("x"), params.get("y"))
                    pyautogui.click(x, y, button='left')
                elif action == "double_click":
                    x, y = self._map_coords(params.get("x"), params.get("y"))
                    pyautogui.doubleClick(x, y)
                elif action == "right_click":
                    x, y = self._map_coords(params.get("x"), params.get("y"))
                    pyautogui.click(x, y, button='right')
                elif action == "type":
                    text = params.get("text", "")
                    if params.get("replace"):
                        if self.system == "darwin":
                            pyautogui.hotkey('command', 'a')
                        else:
                            pyautogui.hotkey('ctrl', 'a')
                        pyautogui.press('backspace')
                    pyautogui.write(text, interval=0.01)
                elif action == "key_combo":
                    keys = params.get("keys", [])
                    pyautogui.hotkey(*keys)
                elif action == "wait":
                    time.sleep(params.get("duration", 1.0))
                elif action == "shell_execute":
                    cmd = params.get("command")
                    p = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=30)
                    res["data"] = {"stdout": p.stdout, "stderr": p.stderr, "returncode": p.returncode}
                elif action == "speak":
                    text = params.get("text")
                    if text: threading.Thread(target=self._speak, args=(text,), daemon=True).start()
                elif action == "focus_app":
                    self._focus_window(params.get("app_name"))
                elif action == "window_move":
                    self._move_window(params.get("title"), params.get("x"), params.get("y"))
                elif action == "window_resize":
                    self._resize_window(params.get("title"), params.get("width"), params.get("height"))
                elif action == "window_maximize":
                    self._maximize_window(params.get("title"))
                elif action == "window_minimize":
                    self._minimize_window(params.get("title"))
                elif action == "clipboard_set":
                    pyperclip.copy(params.get("text", ""))
            except Exception as e:
                res["status"] = "failed"
                res["error"] = str(e)
            results.append(res)
        return results

    def _speak(self, text):
        if self._tts_engine:
            try:
                self._tts_engine.say(text)
                self._tts_engine.runAndWait()
            except Exception: pass

    def _get_window(self, title):
        if not pwc: return None
        # Try exact match
        windows = pwc.getWindowsWithTitle(title)
        if windows: return windows[0]
        # Try fuzzy match
        all_windows = pwc.getAllWindows()
        for w in all_windows:
            if title.lower() in w.title.lower():
                return w
        return None

    def _focus_window(self, name):
        win = self._get_window(name)
        if win:
            try:
                if self.system == "darwin":
                    # macOS often requires a script for reliable activation
                    subprocess.run(['osascript', '-e', f'tell application "{name}" to activate'])
                else:
                    win.activate()
            except Exception: pass

    def _move_window(self, title, x, y):
        win = self._get_window(title)
        if win:
            try: win.moveTo(x, y)
            except Exception: pass

    def _resize_window(self, title, w, h):
        win = self._get_window(title)
        if win:
            try: win.resizeTo(w, h)
            except Exception: pass

    def _maximize_window(self, title):
        win = self._get_window(title)
        if win:
            try: win.maximize()
            except Exception: pass

    def _minimize_window(self, title):
        win = self._get_window(title)
        if win:
            try: win.minimize()
            except Exception: pass

    def get_system_state(self):
        state = {
            "active_window": "",
            "open_windows": [],
            "clipboard_content": "",
            "monitors": [],
            "os": self.system
        }
        try: state["clipboard_content"] = pyperclip.paste()[:500]
        except Exception: pass

        for i, m in enumerate(self._monitors):
            state["monitors"].append({
                "id": i, "width": m["width"], "height": m["height"],
                "left": m["left"], "top": m["top"]
            })

        if pwc:
            try:
                active = pwc.getActiveWindow()
                state["active_window"] = active.title if active else ""
                state["open_windows"] = [w.title for w in pwc.getAllWindows() if w.title]
            except Exception: pass
        return state

executor = Executor()

...
def _process_llm_response_and_create_task(response_data: dict, user_input_obj, thread_instance: Thread, user: User, db: Session):
    if response_data.get('type') == 'desktop_task':
        if user_input_obj.background_mode is True or response_data.get('is_background_mode_requested', False) is True:
    ...
    user_message = ThreadMessage(
        ...
        text=user_input_obj.task if hasattr(user_input_obj, 'task') else user_input_obj.text,
    )
    db.add(user_message)
    db.commit()
    ...
    if response_data.get('type') == 'desktop_task':
        thread_task = ThreadTask(
            thread_id=thread_instance.id,
            task_text=user_input_obj.task if hasattr(user_input_obj, 'task') else user_input_obj.text,
        ...
        thread_instance.status = ThreadStatus.WORKING
        db.add(thread_instance)
        db.commit()
        db.refresh(thread_instance)
...
@router.post('')
async def create_thread(create_thread_obj: CreateThread, db: Session = Depends(get_session),
 ...
 {json.dumps(previous_tasks_arr)}'),
    ...
    return _process_llm_response_and_create_task(response_data, create_thread_obj, instance, user, db)
...
async def send_message(tid: str, obj: SendMessageObj, db: Session = Depends(get_session),
                 user: User = Depends(get_current_user_dependency)):
    instance = db.exec(select(Thread).where(and_(
        Thread.id == tid,
        Thread.user_id == user.id,
    ...
    return _process_llm_response_and_create_task(response_data, obj, instance, user, db)

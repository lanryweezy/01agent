...
async def current_subtask_request(tid: str, 
                            current_subtask_request_obj: CurrentSubtaskRequestObj,
    ...
    if not current_plan:
        previous_tasks = db.exec(select(ThreadTask).where(and_(
            ...
            ThreadTask.status != ThreadStatus.WORKING,
        ...
        llm = get_cached_llm(agent='planner', temperature=0.3)
        ...
        plan_prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content=ai_prompts.PLANNER_AGENT_PROMPT),
...
async def next_step(tid: str, next_step_req: NextStepRequest, db: Session = Depends(get_session),
    ...
    action_history = get_action_history(db, task.id, ThreadChatType.DESKTOP_USE)
